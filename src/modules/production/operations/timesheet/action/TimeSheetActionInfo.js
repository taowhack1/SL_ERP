...
const TimeSheetActionInfo = ({ stepBtn }) => {
  ...
  const listWorker = useMemo(
    () =>
      empList.filter((emp, index) => {
        return form.worker.includes(emp.employee_no);
      }),
    [form.worker]
  );
  const showWorker = useCallback(() => {
    setVisible(true);
    console.log("worker", form.worker);
    console.log("listWorker", listWorker);
  }, [setVisible]);

  const showWorkerConfig = useMemo(
    () => ({
      listWorker: listWorker,
      openModal: visible,
      closeModal: setVisible,
    }),
    [visible, listWorker, setVisible]
  );
  return (
    <>
      <TimeSheetShowWorker {...showWorkerConfig} />
...
export default React.memo(TimeSheetActionInfo);
